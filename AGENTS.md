...
3. HyperFrames runs through `npx hyperframes@0.7.33`, so the first check or render may need npm registry access even when the HyperFrames Skill is available. If `npx` reports `ENETUNREACH`, `EACCES`, `ENOTFOUND`, `ENOTCACHED`, registry access blocked, or a system network policy failure, classify it as an environment/network issue, tell the user that rendering is otherwise ready, ask for permission to access npm, and retry the same check/render automatically after permission. Do not ask the user to repeat the workflow manually. Try the local npm cache with `npx --offline` only as a quick fallback; it cannot replace network access when the package is not cached.
...
Before publication, run `npm run check`, granting npm registry access and retrying automatically if HyperFrames is not cached, then scan reachable Git history for secrets and private media, verify no full reference transcript remains, and confirm a clean clone initializes with and without WeChat Reading.
