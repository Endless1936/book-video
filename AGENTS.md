...
1. Check Codex capabilities first: the HyperFrames plugin/Skill and built-in bitmap image generation are capabilities, not user-installed project dependencies. Use them directly when available; do not ask the user to install a separate image model or HyperFrames Skill.
2. Check local runtime prerequisites: Node.js 22+, `npx`, FFmpeg/FFprobe, and `whisper-cli`. If any are missing, tell the user exactly which ones are missing and ask for confirmation before installing them. After confirmation, the Agent may install them with the available platform package manager; never install or change system packages silently. The repository does not auto-install these through a project script.
3. Check whether `assets/models/whisper/ggml-base.bin` exists. If missing, ask to download it as part of the confirmed setup, then run `node scripts/download-whisper-model.mjs`. If the download fails, first look for an enabled computer proxy or proxy environment variable; use it for a retry with `--proxy`, asking the user to enable their proxy when none is active. Do not change system network settings silently. If that fails, give the user the browser URL `https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-base.bin`, then install the user-provided file with `node scripts/download-whisper-model.mjs --from "<local-path>"`.
4. Run `scripts/init.mjs`. It creates local state and the private pipeline file, asks once whether to enable WeChat Reading, and reports missing prerequisites.
5. If WeChat Reading is confirmed, install the official Tencent skill locally or through the agent's skill installer. Do not vendor its source into this repository.
6. After installation, guide the user to [微信读书 Skills 官网](https://weread.qq.com/r/weread-skills) to obtain the API Key. Collect `WEREAD_API_KEY` only through hidden TTY input or a local `.env` file with mode `0600`. Never log the key and never accept it as a command argument.
7. If WeChat Reading is declined or unavailable, continue with public research or a user-provided title.
