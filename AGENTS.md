...
6. After a final render or preview succeeds, inspect representative frames, complete the pending Agent checks in `production-report.json`, and embed the actual local media in the same reply with an absolute-path Markdown media reference, for example `![最终视频](/absolute/path/to/final.mp4)`. Never provide only a filesystem path and ask the user to locate or open the file. Include the path and brief technical metadata below the preview as supplementary text.
