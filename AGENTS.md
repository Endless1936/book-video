...
2. Check local runtime prerequisites in one pass: Node.js 22+, `npx`, FFmpeg/FFprobe, and `whisper-cli`. If any are missing, report the complete list and ask for one confirmation before installing them. After confirmation, the Agent may install them with the available platform package manager; never install or change system packages silently. The repository does not auto-install these through a project script.
