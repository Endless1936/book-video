...
1. 初始化：先确认 Codex 已启用 HyperFrames 和图片生成能力，再检查 Node.js、FFmpeg、`whisper-cli` 和 Whisper 模型。缺少本机运行时依赖时，Agent 先说明缺项并征得用户同意，再负责安装；不通过项目脚本静默安装。Whisper 下载失败时按代理重试、浏览器下载、本地接入的顺序处理。微信读书 Skill 为可选项。
