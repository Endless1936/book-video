...
5. If `WEREAD_API_KEY` is missing, ask only whether the user wants to configure the integration. After confirmation, open [微信读书 Skills 官网](https://weread.qq.com/r/weread-skills) with the browser/computer tool, then collect the key through hidden input. Store it in local `.env` with mode `0600`; never log the key or accept it as a command argument. If the user declines key configuration, continue with public research.
